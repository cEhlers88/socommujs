    ...
    switch (event) {
      case EServerEvent.clientWillConnect:
          ...
          newClient.Connection.on('close',()=>{
            ...
            self.DataHandler.getData("_Serverdata")._Eventhandler.dispatch(getServereventString(EServerEvent.clientDisconnected),newClient);
              ...
              message.data.map((Data:IDataEntry)=>{
                ...
                if(this.checkLogin(username,password)){
                  client.messageHandle=(request:any)=>{
                      self.DataHandler.getData('_Serverdata')._Eventhandler.dispatch(getServereventString(EServerEvent.getAuthenticatedRequest),JSON.parse(request.utf8Data));
                  };
                }
              break;
          case UserExist:
